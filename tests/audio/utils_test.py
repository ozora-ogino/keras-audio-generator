...
def test_validate_filename(tmpdir):
    ...
    filename = tmpdir.ensure('test.wav')
    assert utils.validate_filename(str(filename), valid_extensions)

    filename = tmpdir.ensure('test.Flac')
    assert utils.validate_filename(str(filename), valid_extensions)

    filename = tmpdir.ensure('test.some_extension')
    assert not utils.validate_filename(str(filename), valid_extensions)
    assert not utils.validate_filename('some_test_file.wav', valid_extensions)


def test_load_audio(tmpdir):
    filename = str(tmpdir / 'test.wav')

    original_array = np.array(255 * np.random.rand(100),
                                  dtype=np.float32)
    original = utils.array_to_audio(original_array, scale=False)
    ...
    loaded_im_array = utils.img_to_array(loaded_im) assert loaded_im_array.shape == (25, 25, 1)
